...
exports.ALLOWED_TABLES = void 0;
exports.listTables = listTables;
exports.buildWhere = buildWhere;
exports.previewRows = previewRows;
exports.streamRows = streamRows;
// =============================================================================
...
function getPool() {
    if (!pool) {
        if (!ANALYTICS_DB_URL) {
            throw new Error('ANALYTICS_DB_URL is not set — admin cannot reach the analytics Postgres. ' +
...
function assertAllowedTable(t) {
    if (!exports.ALLOWED_TABLES.includes(t)) {
 ...
 */
function buildWhere(table, cols, filters) {
    const colNames = new Set(cols.map((c) => c.name));
    const parts = [];
    const params = [];
    for (const f of filters ?? []) {
        if (!colNames.has(f.column))
            throw new Error(`unknown column: ${f.column}`);
 ...
 */
async function previewRows(opts) {
...
async function* streamRows(opts) {
    ...
    try {
        ...
        const res = await client.query(`SELECT * FROM public.${tableQ} ${where} LIMIT ${cap}`, params);
